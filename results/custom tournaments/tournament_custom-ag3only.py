...
def play_round(agents, num_matches, data_frame):
    ...
    print("\nPlaying Matches:")
    print("----------")
    with open("results_ag2_only.txt", "a") as myfile:
...
def main():
    ...
    # We only compare agressiveness of 2 , that go the best results in previous tournaments, to ID_Improved but this time over 100 matches
    id_op = [Agent(CustomPlayer(score_fn=improved_score, **CUSTOM_ARGS), "ID_Improved")]

    test_agents = [
        Agent(CustomPlayer(score_fn=custom_score_generator(2), method='alphabeta', iterative=True, agressiveness=2, search_depth=1), "Student ag2"),
    ]
